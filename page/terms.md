...
## 第2条（codocによるサポート機能）
1. 当サイトは，codoc株式会社が提供するサービス「codoc」を利用し，ユーザーが任意に選択した金額で番組をサポートできる機能（以下，「サポート機能」といいます。）を提供します。
2. サポート機能の利用には，本規約に加えて，[codoc利用規約](https://codoc.jp/ja/docs/termsofuse)および[codocプライバシーポリシー](https://codoc.jp/ja/docs/privacy)が適用されます。
3. サポート機能に関する取引は，ユーザーと当サイトとの間で成立し，codoc株式会社は当サイトに代わって代金を受領します。
4. 支払金額および支払方法は，codocの購入画面に表示される内容に従うものとします。
5. 支払い完了後の返金は，法令上必要な場合またはcodoc利用規約により認められる場合を除き，原則として行いません。
6. codocの障害，仕様変更またはサービス終了等によってサポート機能を利用できない場合について，当サイトは責任を負わないものとします。ただし，当サイトの故意または重過失による場合を除きます。
